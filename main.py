...
n = np.arange(1, C_UPPER, C_STEP)
vec = np.arange(0, C_AVG_AMOUNT, C_STEP)

time_f = []
for i in n - 1:
    avg_time = 0
    V_rand = np.random.uniform(0, 1, n[i])
    # Uncomment for matrices
    A = np.random.randn(10, 10)
    B = np.random.randn(10, 10)
    for k in vec:
        t_previous = perf_counter()
        # matmult(A,B)               #Very slow
        # Matrix_Mult_native(A, B)
        #strassen(A, B)
        #C = strass(A, B)
        C = strass(A, B)
        #print_matrix(C)
        avg_time += (perf_counter() - t_previous)
    time_f.append(avg_time / C_AVG_AMOUNT)

time_ob = lambda x: x * pow(10, -7) * 3.62

x = np.arange(1, C_UPPER, 1)
plt.title('Fig.1 -')
plt.plot(n, time_f, 'b-', linewidth=1.0)
plt.plot(x, time_ob(x), 'r-', linewidth=1.0, alpha=0.8)
plt.xlabel('n')
plt.ylabel('Time(seconds)')
plt.legend(["Empirical time", "Theoretical time"], loc="upper left")
plt.show()
